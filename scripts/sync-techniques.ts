/**
 * Sync Product Techniques and Placements
 *
 * Standalone script to sync techniques and placements from Printful API to database.
 * Run with: npx tsx scripts/sync-techniques.ts
 */
  ...
  try {
    ...
    for (const product of products) {
      try {
        ...
        // Fetch mockup styles from Printful to discover placements
        ...
        if (styles.length === 0) {
          console.log(`[Sync] No mockup styles found for ${product.name}, skipping`);
          continue;
        }

        // Collect all unique placements across all styles
        const allPlacements = new Set<string>();
        for (const style of styles) {
          for (const placement of style.placements || []) {
